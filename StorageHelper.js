...
export const getSavedFiles = async () => {
    const savedPuzzles = await AsyncStorage.getItem(PUZZLE_KEY);
...
export const updateTotalAvgLenForTime = async(time, newAvgLen) => {
    const totalAvgLen = await getStatForKey(TOTAL_AVG_LEN_PREFIX + time);
    await AsyncStorage.setItem(TOTAL_AVG_LEN_PREFIX + time, JSON.stringify(totalAvgLen + newAvgLen));
...
export const updateAccuracy = async (time, attempts, posAttempts) => {
    const prevAttempts = await getStatForKey(ATTEMPTS_PREFIX + time);
    const prevPosAttempts = await getStatForKey(POS_ATTEMPTS_PREFIX + time);
    await AsyncStorage.setItem(ATTEMPTS_PREFIX + time, JSON.stringify(prevAttempts + attempts));
    await AsyncStorage.setItem(POS_ATTEMPTS_PREFIX + time, JSON.stringify(prevPosAttempts + posAttempts));
