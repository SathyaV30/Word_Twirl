...
export default function PostGameMultiplayer({ route, navigation }) {
  ...
  const { allWords, foundWords, userScore, selectedTime, letters, wordsToPath, room, opponent } = route.params;
  ...
  useEffect(() => {
    ...
    socket.on('opponentPostGameData', (opponentGameData) => {
      ...
      const opponentScore = opponentFoundWords.reduce((acc, word) => acc + word.length ** 2, 0);  
      ...
      <SafeAreaView>
        <View style={styles.mainContainer}>
          ...
          <View style={styles.scrollViewsContainer}>
            <ScrollView style={styles.scrollView}>
              ...
              {sortedFoundWords.map((word, index) => (
                <TouchableOpacity style={styles.wordContainer} key={index} onPress={() => navigation.navigate('WordDetailsScreen', { word: word.toLowerCase(), letters, wordsToPath, fromGame: true })}>
          ...
          <View style={styles.buttonContainer}>
            <TouchableOpacity style={styles.button} onPress={() => { navigation.navigate('Start Screen', { multiplayer: true }); playButtonSound(isSoundMuted); }}>
